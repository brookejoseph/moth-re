...
export function resolveDefaultOut(binaryPath) {
  const absolute = resolveBinaryInput(binaryPath).binaryPath;
...
export function analyzeBinary(binaryPath, options = {}) {
  const input = resolveBinaryInput(binaryPath);
  const absolute = input.binaryPath;
...
export function resolveBinaryInput(binaryPath) {
  const inputPath = path.resolve(binaryPath);
  ...
  if (!stat.isDirectory()) {
    return { inputPath, binaryPath: inputPath, bundlePath: null };
  }

  if (!inputPath.endsWith(".app")) {
    throw new Error(`Input is a directory, not a binary: ${inputPath}`);
  }

  const macosDir = path.join(inputPath, "Contents", "MacOS");
  if (!fs.existsSync(macosDir)) {
    throw new Error(`App bundle has no Contents/MacOS directory: ${inputPath}`);
  }

  const executableName = readBundleExecutableName(inputPath);
  const candidates = executableName
    ? [path.join(macosDir, executableName)]
    : fs.readdirSync(macosDir).map((entry) => path.join(macosDir, entry));

  const resolvedBinaryPath = candidates.find((candidate) => {
    if (!fs.existsSync(candidate)) return false;
    const candidateStat = fs.statSync(candidate);
    return candidateStat.isFile() && (candidateStat.mode & 0o111);
  });

  if (!resolvedBinaryPath) {
    throw new Error(`Could not find an executable in ${macosDir}`);
  }

  return { inputPath, binaryPath: resolvedBinaryPath, bundlePath: inputPath };
}

function readBundleExecutableName(bundlePath) {
